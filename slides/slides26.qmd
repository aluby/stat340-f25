...
$$
\begin{aligned}
Y_{i_j} &\sim {N}(\mu_{i_j}, \sigma_y) & \text{Race times within runners } j \\
\mu_{i_j} &= \beta_{0_j} + \beta_1 X_{i_j} & \text{Linear model of within-runner variation} \\
\beta_{0_j} &\sim {N}(\beta_0, \sigma_0) & \text{Variability in average times between runners} \\
\\
...
b_{0_j} &\sim {N}(0, \sigma_0) & \text{Random runner offsets} \\
\\
\beta_0 &\sim \text{Some prior} & \text{Global priors} \\
\beta_1 &\sim \text{Some prior} \\
\sigma_y &\sim \text{Some prior} \\
...
save(model_running_ranint, file = "26-ranint.rda")
```
...
\\
\beta_{0_c} &\sim {N}(100, 10) & \text{Prior for global average} \\
\beta_1 &\sim {N}(2.5, 1) & \text{Prior for global age effect} \\
...
```
![](https://bayesf22-notebook.classes.andrewheiss.com/bayes-rules/17-chapter_files/figure-html/unnamed-chunk-23-1.png)
...
- `reg` = LKJ parameter
- `conc` = concentration hyperparameter for dirichlet distribution on variance components
- `shape` and `scale` = gamma hyperparameters for individual variance components
...
save(model_running_ranslope, file = "26-ranslope.rda")
```

```{r}
load("26-ranslope.rda")
```
...
- $\beta_0$ is the overall global intercept for all runners. There's an 80% chance that it's between 3.5 and 33.4 minutes.
...
head(runner_summaries_ranslope)
```
...
runners |>
  ...
  geom_point() +
  geom_abline(data = runner_summaries_ranslope |>
                      mutate(runner = str_remove(runner, "runner:")) |>
                filter(runner %in% 25:36), 
              color = "gray",
...
```{r}
p1 <- pp_check(model_running_ranint) + labs(title = "Random Intercepts")
p2 <- pp_check(model_running_ranslope) + labs(title = "Random Slopes")
...
set.seed(84735)
prediction_summary(model = model_running_ranint, data = runners)
prediction_summary(model = model_running_ranslope, data = runners)
```
...
elpd_hierarchical_1 <- loo(model_running_ranint)
elpd_hierarchical_2 <- loo(model_running_ranslope)
